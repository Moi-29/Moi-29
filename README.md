<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:00FFC6,100:7E00FF&height=220&section=header&text=⚡%20Moibon%20Berhau%20⚡&fontSize=50&animation=fadeIn&fontColor=FFFFFF&fontAlignY=38" />
</p>

<table align="center" border="0">
  <tr>
    <td align="center" valign="center">
      <img src="https://raw.githubusercontent.com/Moi-29/Moi-29/main/profile.png" width="160" style="border-radius: 50%; border: 4px solid #00FFC6; box-shadow: 0 0 20px #7E00FF;" />
    </td>
    <td align="left" valign="center">
      <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=28&pause=1000&color=00FFC6&center=false&vCenter=true&width=450&lines=Frontend+Developer;Creative+UI+Engineer;HTML+•+CSS+•+JS+•+React.js;Turning+ideas+into+modern+UI" />
    </td>
  </tr>
</table>

<p align="center">
  <img src="https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/aqua.png" width="100%">
</p>

<table align="center">
  <tr>
    <td width="50%" valign="top">
      <h2 align="center">🚀 About Me</h2>
      <ul>
        <li>🔭 <b>Building:</b> Frontend projects + UI systems</li>
        <li>🌱 <b>Learning:</b> Adv. JavaScript & Node.js</li>
        <li>👯 <b>Collaboration:</b> React apps & Open Source</li>
        <li>💬 <b>Ask me:</b> CSS Architecture & React Logic</li>
        <li>⚡ <b>Fact:</b> Debugging is my favorite puzzle</li>
      </ul>
    </td>
    <td width="50%" valign="top">
      <h2 align="center">📊 Real-time Stats</h2>
      <img src="https://github-readme-stats.vercel.app/api?username=Moi-29&show_icons=true&theme=radical&icon_color=00FFC6&title_color=00FFC6&border_color=7E00FF&bg_color=0D1117" width="100%"/>
    </td>
  </tr>
</table>

---

<div align="center">
  <h2>🧠 Tech Stack Universe</h2>
  <img src="https://skillicons.dev/icons?i=html,css,js,react,tailwind,bootstrap,nodejs,express,mongodb,ts,git,github,vscode,figma&perline=7&theme=dark" />
</div>

<br>

---

<div align="center">
  <h2>📈 Coding Activity & Performance</h2>
  <p><i>(Fixed: Using Activity Graph for high-performance visual)</i></p>
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=Moi-29&theme=tokyo-night&bg_color=0D1117&color=00FFC6&line=7E00FF&point=FFFFFF&hide_border=true" width="95%"/>
</div>

<div align="center">
  <table>
    <tr>
      <td><img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Moi-29&layout=compact&theme=radical&title_color=00FFC6&border_color=7E00FF&bg_color=0D1117" /></td>
      <td><img src="https://streak-stats.demolab.com/?user=Moi-29&theme=tokyonight&ring=00FFC6&fire=7E00FF&hide_border=true" /></td>
    </tr>
  </table>
</div>

---

<h2 align="center">📁 Featured Masterpieces</h2>

<div align="center">
<table width="100%">
  <tr>
    <td width="50%" align="center">
      <img src="https://capsule-render.vercel.app/api?type=rect&color=1a1a1a&height=100&section=header&text=Job-Intern%20Website&fontSize=20&fontColor=00FFC6" width="100%"/>
      <p>Modern job platform with seamless UI</p>
      <a href="#"><b>View Project →</b></a>
    </td>
    <td width="50%" align="center">
      <img src="https://capsule-render.vercel.app/api?type=rect&color=1a1a1a&height=100&section=header&text=E-Commerce%20UI&fontSize=20&fontColor=7E00FF" width="100%"/>
      <p>High-performance shopping experience</p>
      <a href="#"><b>View Project →</b></a>
    </td>
  </tr>
</table>
</div>

---

<div align="center">
  <h2>🐍 My Contribution Journey</h2>
  <img src="https://raw.githubusercontent.com/trepichio/trepichio/output/github-contribution-grid-snake-dark.svg" />
</div>

<br>

<div align="center">
  <h2>🌐 Let's Connect</h2>
  <a href="mailto:moibonberhau6@gmail.com">
    <img src="https://img.shields.io/badge/Email-00FFC6?style=for-the-badge&logo=gmail&logoColor=000" />
  </a>
  <a href="https://github.com/Moi-29">
    <img src="https://img.shields.io/badge/GitHub-7E00FF?style=for-the-badge&logo=github&logoColor=fff" />
  </a>
  <br><br>
  <a href="https://buymeacoffee.com/">
    <img src="https://img.shields.io/badge/Buy%20Me%20A%20Coffee-FFDD00?style=for-the-badge&logo=buy-me-a-coffee&logoColor=black" />
  </a>
  <br><br>
  <img src="https://komarev.com/ghpvc/?username=Moi-29&style=for-the-badge&color=00FFC6" />
</div>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:7E00FF,100:00FFC6&height=120&section=footer" />
</p>
